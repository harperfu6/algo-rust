...
impl Node {
    ...
    fn neighbors(&self, graph: &Graph) -> VecDeque<Node> {
        graph
            .edges
            .iter()
            .filter(|e| e.0 == self.0)
...
/// Perform a depth-first search on Graph `graph`.
///
/// # Parameters
///
...
/// `None` is returned.
///
fn depth_first_search(graph: &Graph, root: Node, target: Node) -> Option<Vec<u32>> {
    let mut queue = VecDeque::new(); // DFSの実態(双方向のキュー)
    ...
    while let Some(currentnode) = queue.pop_front() {
        ...
        // 左から読み込む場合revをつける
        for neighbor in currentnode.neighbors(graph).into_iter().rev() {
            if visited.insert(neighbor) {
                queue.push_front(neighbor); // ここでエッジをどんどん辿ってキューの先頭に追加し探索の優先度を上げるのがポイント(LIFO)
            }
        }
    }
...
mod tests {
    ...
    fn not_found_node() {
        ...
        assert_eq!(
            depth_first_search(&graph1, root.into(), target.into()),
            None
        );
    }
    ...
    fn all_node() {
        ...
        let expected_history = vec![1, 2, 4, 5, 8, 3, 6, 7];
